...
function EventScreen.new(event)
	...
	self.topText = HighlightTextbox.new(self.topText)
	...
	do
		local newTextbox = HighlightTextbox.new(WordDownshiftTextbox.new(option:get_text(), event_text_font, nil, Screen.width / 2))
	...
	self._options_collection:setY(self.topText:getY() + self.topText:getHeight())
