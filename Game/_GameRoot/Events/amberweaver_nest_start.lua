...
function amberweaver_jackpot_start.get_text(self, game_data)
	return "When looking for a cave to set up camp for the night, you come across a cocoon structure composed of grey threads. It's hard to the touch. "
end

function amberweaver_jackpot_start.new()
	local self = Event.new("amberweaver_nest_start")
	setmetatable(self, {__index = amberweaver_jackpot_start})

	local option_0 = Option.new("Break it open. ", false)
	option_0.get_next_event_name = function(self, game_data) return "amberweaver_active_nest" end 
